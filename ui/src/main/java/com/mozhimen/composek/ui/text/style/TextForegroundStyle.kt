...
import com.mozhimen.composek.ui.text.lerpDiscrete

/**
 * @ClassName TextForegroundStyle
 * @Description TODO
 * @Author mozhimen
 * @Date 2024/10/30
 * @Version 1.0
...
internal fun lerp(
    ...
    return if ((start !is BrushStyle && stop !is BrushStyle)) {
        TextForegroundStyle.from(androidx.compose.ui.graphics.lerp(start.color, stop.color, fraction))
    } else if (start is BrushStyle && stop is BrushStyle) {
        TextForegroundStyle.from(
            lerpDiscrete(start.brush, stop.brush, fraction),
            androidx.compose.ui.util.lerp(start.alpha, stop.alpha, fraction)
        )
    } else {
