...
internal class OutlineResolver(private var density: Density) {
    ...
    private var calculatedOutline: androidx.compose.ui.graphics.Outline? = null
    ...
    private fun updateCache() {
        if (cacheIsDirty) {
            ...
            if (outlineNeeded && size.width > 0.0f && size.height > 0.0f) {
                ...
                when (outline) {
                    is androidx.compose.ui.graphics.Outline.Rectangle -> updateCacheWithRect(outline.rect)
                    is androidx.compose.ui.graphics.Outline.Rounded -> updateCacheWithRoundRect(outline.roundRect)
                    is androidx.compose.ui.graphics.Outline.Generic -> updateCacheWithPath(outline.path)
                }
            } else {
